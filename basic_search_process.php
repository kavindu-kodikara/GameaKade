...
if (!empty($txt) && $select == 0) {
    $query = "`product` WHERE `title` LIKE '%" . $txt . "%'";
} elseif (empty($txt) && $select > 0) {
    $query = "`product` WHERE `category_id` ='" . $select . "'";
} elseif (!empty($txt) && $select > 0) {
    $query = "`product` WHERE `title` LIKE '%" . $txt . "%' AND `category_id` ='" . $select . "'";
}

$p_rs = Database::search("SELECT * FROM  " . $query . "");
$p_num = $p_rs->num_rows;
